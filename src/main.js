...
function etlChecksumForPacket(packetWithoutChecksum) {
  const sum = [...packetWithoutChecksum].reduce(
    (a, c) => a + (c.charCodeAt(0) - 32),
    0
  );
...
class VictorInstance extends InstanceBase {
  async init(config) {
    ...
    this.log(
      "info",
      "Victor ready. Test Connect to verify. Aliases and routing status will auto update."
    );
  }
    ...
    this.setActionDefinitions({
      ...
      route: {
        ...
        callback: async ({ options }) => {
          const inStr = await this.parseVariablesInString(
            String(options.input ?? "")
          );
          const outStr = await this.parseVariablesInString(
          ...
          if (
            ...
            iNum < 0 ||
            iNum > Math.max(999, maxIn)
          ) {
            this.log(
              "error",
              `Input must be a number 0..${maxIn}. Got "${inStr}"`
            );
            return;
          }
          if (
            ...
            oNum < 1 ||
            oNum > Math.max(999, maxOut)
          ) {
            this.log(
              "error",
              `Output must be a number 1..${maxOut}. Got "${outStr}"`
            );
            return;
          }
      ...
      route_pair: {
        ...
        callback: async ({ options }) => {
          const inStr = await this.parseVariablesInString(
            String(options.input_odd ?? "")
          );
          const outStr = await this.parseVariablesInString(
            String(options.output_odd ?? "")
          );
          ...
          if (!Number.isFinite(i1) || i1 < 1 || i1 > maxIn || i1 % 2 === 0) {
            this.log(
              "error",
              `Input must be an odd number 1..${maxIn}. Got "${inStr}"`
            );
            return;
          }
          if (!Number.isFinite(o1) || o1 < 1 || o1 > maxOut || o1 % 2 === 0) {
            this.log(
              "error",
              `Output must be an odd number 1..${maxOut}. Got "${outStr}"`
            );
            return;
          }
          if (i1 + 1 > maxIn) {
            this.log(
              "error",
              `Input pair overflows. Need input ${i1 + 1} to exist`
            );
            return;
          }
          if (o1 + 1 > maxOut) {
            this.log(
              "error",
              `Output pair overflows. Need output ${o1 + 1} to exist`
            );
            return;
          }
  ...
  async pollAliasesOnce() {
    ...
    const changed =
      outAliases.length !== this.outputsCount ||
      inAliases.length !== this.inputsCount;
    ...
    const vals = {};
    outAliases.forEach(
      (name, idx) => (vals[`output_${pad3(idx + 1)}_name`] = name)
    );
    inAliases.forEach(
      (name, idx) => (vals[`input_${pad3(idx + 1)}_name`] = name)
    );
    ...
    return [
      {
        ...
        label: "Info",
        value:
          "Host and port. Port default is 4000. DA and SA usually A and B.",
      },
      {
        type: "textinput",
        id: "host",
